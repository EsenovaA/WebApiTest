        ...
        public static PagedResponse<List<T>> CreatePagedReponse<T>(List<T> pagedData, Filter validFilter, int totalRecords, IUriService uriService, string route)
        {
            ...
            respose.PreviousPage =
                validFilter.PageNumber - 1 >= 1 && validFilter.PageNumber <= roundedTotalPages
                    ? uriService.GetPageUri(new Filter(validFilter.PageNumber - 1, validFilter.PageSize), route)
                    : null;
            respose.FirstPage = uriService.GetPageUri(new Filter(1, validFilter.PageSize), route);
            respose.LastPage = uriService.GetPageUri(new Filter(roundedTotalPages, validFilter.PageSize), route);
